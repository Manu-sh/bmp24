...
constexpr uint32_t bits2bytes(uint32_t bits) noexcept {
	return bits >> 3;
}

// get rowsize including padding
constexpr int32_t row_size(int32_t width, ColorDepth depth) noexcept {
	return ((width * depth + 31) / 32) * 4;
}

// tested only with truecolor
// width%4 == 0 ? 4 : ((width/4) + 1) * 4
constexpr uint32_t row_padding(int32_t width, ColorDepth depth) noexcept {
